...
export default function Chat() {
  ...
  const sendMessage = async () => {
    ...
    setLoading(true)
    await supabase.from('messages').insert({ user_id: user.id, content: input, sender_name: user.user_metadata?.name || 'You' })
    setInput('')
    setLoading(false)
    setTimeout(async () => {
      await supabase.from('messages').insert({ user_id: 'ai', content: `Thanks! You said: "${input}"`, sender_name: 'Wakil AI' })
    }, 1000)
  }
    ...
    <>
      ...
      <div className="h-screen bg-gray-900 flex flex-col">
        ...
        <div className="bg-gray-800 p-4 flex gap-2">
          <input value={input} onChange={e => setInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && sendMessage()} placeholder="Type a message..." className="flex-1 bg-gray-700 text-white rounded-full px-4 py-3" />
          <button onClick={sendMessage} disabled={loading} className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-3 rounded-full font-bold">{loading ? '...' : 'Send'}</button>
        </div>
      </div>
    </>
  )
